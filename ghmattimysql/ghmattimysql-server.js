! function(modules) {
    ...
    var LogLevel, logger_writeConsole = function(msg, options) {
        ...
        if (tag.includes("ETIMEDOUT")) {
            console.log(`^1[ghmattimqsql]^1:^7 The server took long to load but do not worry, a connection will be established. If you see this away from server startup the connection is timing out.`)
            return;
        }
        if (msg.includes("ECONNREFUSED")) {
            console.log(`^1[ghmattimqsql]^1:^7 The database connection could not be established. Make sure to edit the ghmattimysql/config.json with the correct credentials.`)
            return;
        }
        console.log(`${tag} ${msg}`)
    };
